using BinaryBuilder, HTTP, Compat
#import Sockets
...
if [[ "\$COMPILER_TARGET" == *apple* ]]; then
    ...
    unset LDFLAGS
    #export AS="llvm-clang-as"
    export AS=\$(which \${COMPILER_TARGET}-as)

    #echo '#!/bin/bash' > /usr/local/bin/llvm-clang-as
    #echo "\$(which \${COMPILER_TARGET}-as) -Q" >> /usr/local/bin/llvm-clang-as
    #chmod +x /usr/local/bin/llvm-clang-as
